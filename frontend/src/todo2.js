...
var $author$project$Todo2$defaultTimeSlotLength = (15 * 60) * 1000;
var $author$project$Todo2$defaultTodo = {content: 'Make a Todo!', length: $author$project$Todo2$defaultTimeSlotLength, startTime: 0};
...
var $author$project$Todo2$GetNewTime = function (a) {
	return {$: 'GetNewTime', a: a};
};
var $author$project$Todo2$GetTimeZone = function (a) {
...
var $author$project$Todo2$initializeTime = $elm$core$Platform$Cmd$batch(
	_List_fromArray(
		[
			A2($elm$core$Task$perform, $author$project$Todo2$GetNewTime, $elm$time$Time$now),
			A2($elm$core$Task$perform, $author$project$Todo2$GetTimeZone, $elm$time$Time$here)
		]));
var $elm$time$Time$utc = A2($elm$time$Time$Zone, 0, _List_Nil);
var $author$project$Todo2$init = function (_v0) {
	var blankModel = {
		newTodo: _Utils_update(
...
var $author$project$Todo2$subscriptions = function (model) {
...
var $author$project$Todo2$getSelectableTimes = function (model) {
	...
	var timeGaps = A2(
		...
		function (x) {
			return (_Utils_cmp(x.startTime, x.endTime) < 0) && (_Utils_cmp(x.startTime, roundedTime) > 0);
		},
		A2(
...
var $author$project$Todo2$noteDecoderResultsHandler = function (result) {
	if (result.$ === 'Err') {
		var error = result.a;
		var _v1 = A2($elm$core$Debug$log, 'Notes error is ', error);
		return $elm$core$Array$fromList(_List_Nil);
	} else {
		var noteArray = result.a;
		return noteArray;
	}
};
	...
	var encodedVector2 = $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				_Utils_Tuple2(
				'x',
				$elm$json$Json$Encode$int(note.position.x)),
				_Utils_Tuple2(
				'y',
				$elm$json$Json$Encode$int(note.position.y))
			]));
...
var $author$project$Todo2$encodeNoteArray = function (noteArray) {
	return A2($elm$json$Json$Encode$array, $author$project$Todo2$noteEncoder, noteArray);
};
...
var $author$project$Todo2$todoEncoder = function (todo) {
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				_Utils_Tuple2(
				'content',
				$elm$json$Json$Encode$string(todo.content)),
				_Utils_Tuple2(
				'startTime',
				$elm$json$Json$Encode$int(todo.startTime)),
				_Utils_Tuple2(
				'length',
				$elm$json$Json$Encode$int(todo.length))
			]));
};
var $author$project$Todo2$encodeTodoList = function (todoList) {
...
var $author$project$Todo2$ModifyDOM = function (a) {
	return {$: 'ModifyDOM', a: a};
};
...
var $author$project$Todo2$resetScrollPosition = function (id) {
	return A2(
		$elm$core$Task$attempt,
		$author$project$Todo2$ModifyDOM,
		A3($elm$browser$Browser$Dom$setViewportOf, id, 0, 0));
};
var $elm$core$List$sortBy = _List_sortBy;
var $author$project$Todo2$Todo = F3(
	function (content, startTime, length) {
		return {content: content, length: length, startTime: startTime};
	});
var $author$project$Todo2$todoDecoder = A4(
	$elm$json$Json$Decode$map3,
	$author$project$Todo2$Todo,
	A2($elm$json$Json$Decode$field, 'content', $elm$json$Json$Decode$string),
	A2($elm$json$Json$Decode$field, 'startTime', $elm$json$Json$Decode$int),
	A2($elm$json$Json$Decode$field, 'length', $elm$json$Json$Decode$int));
var $author$project$Todo2$todoDecoderResultsHandler = function (result) {
	if (result.$ === 'Err') {
		var error = result.a;
		var _v1 = A2($elm$core$Debug$log, 'Todo error is ', error);
		return _List_Nil;
	} else {
		var todoList = result.a;
		return todoList;
	}
};
	...
	function (msg, model) {
		switch (msg.$) {
			...
			case 'NewTodoLength':
				...
				return _Utils_Tuple2(
					_Utils_update(
						...
						{
							newTodo: {
								content: model.newTodo.content,
								length: newLength,
								startTime: $author$project$Todo2$getNextFreeTime(model)
							}
						}),
					$author$project$Todo2$resetScrollPosition('timeSelection'));
			case 'SubmitNewTodo':
				var newTodo = msg.a;
				var newModel = _Utils_update(
					model,
					{
						todoList: A2(
							...
							A2(
								$elm$core$List$cons,
								A2($author$project$Todo2$cleanNewTodo, newTodo, model),
								model.todoList))
					});
				return _Utils_Tuple2(
					_Utils_update(
					...
					$author$project$Todo2$postTodoList(newModel.todoList));
			case 'RemoveTodo':
				var todo = msg.a;
				var newTodoList = A2(
					$elm$core$List$filter,
					function (x) {
						return !_Utils_eq(x, todo);
					},
					model.todoList);
				var newModel = _Utils_update(
					model,
			...
			case 'UpdateTodoList':
				...
				return _Utils_Tuple2(
					newModel,
					$author$project$Todo2$postTodoList(newTodoList));
			case 'SpawnNote':
				var position = msg.a;
				var newNote = {
					content: '',
					createdTime: $elm$time$Time$posixToMillis(model.time.now),
					position: position
				};
				var newModel = _Utils_update(
					model,
						...
						_Utils_update(
							model,
							{
								todoList: A2(
									...
									$author$project$Todo2$todoDecoderResultsHandler(
										A2(
											$elm$json$Json$Decode$decodeValue,
											$elm$json$Json$Decode$list($author$project$Todo2$todoDecoder),
											todoListJson)))
							}),
						$elm$core$Platform$Cmd$none);
				} else {
...
var $author$project$Todo2$SubmitNewTodo = function (a) {
	return {$: 'SubmitNewTodo', a: a};
};
...
var $author$project$Todo2$lengthSelectionGenerator = function (model) {
	var selectableLengths = A2(
		$elm$core$List$map,
		function (x) {
			return x * $author$project$Todo2$defaultTodo.length;
		},
		A2($elm$core$List$range, 1, 9));
	var selectableLengthsHtml = A2(
		$elm$core$List$map,
		function (newLength) {
			return A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
						...
						$author$project$Todo2$NewTodoLength(newLength))
					]),
				_List_fromArray(
					[
						A2(
							...
							[
								...
								_List_fromArray(
									[
										...
										_Utils_eq(model.newTodo.length, newLength)),
										$elm$html$Html$Attributes$name('lengthSelector'),
										$elm$html$Html$Attributes$type_('radio')
									]),
								_List_Nil),
								$elm$html$Html$text(
								$author$project$Todo2$countdownStringGenerator(newLength))
							]))
					]));
		},
		selectableLengths);
	return A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$Attributes$id('lengthSelection'),
				A2($elm$html$Html$Attributes$style, 'display', 'inline-block'),
				A2($elm$html$Html$Attributes$style, 'width', '100%'),
				A2($elm$html$Html$Attributes$style, 'height', '1.3em'),
...
var $author$project$Todo2$noteEditor = F3(
	function (note, model, newContent) {
		var noteIndex = A3(
			$elm$core$List$foldl,
			$elm$core$Basics$add,
			0,
...
var $author$project$Todo2$noteGenerator = F2(
	function (note, model) {
		var y = $elm$core$String$fromInt(note.position.y) + 'px';
		var x = $elm$core$String$fromInt(note.position.x) + 'px';
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
						...
						[
							...
							_List_fromArray(
								[
									...
									A2($author$project$Todo2$noteEditor, note, model))
								]),
							_List_fromArray(
								[
									$elm$html$Html$text(note.content)
								]))
						]))
				]));
	});
var $author$project$Todo2$noteArrayGenerator = function (model) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
...
var $author$project$Todo2$timeStringGenerator = F2(
	function (model, timePosix) {
		...
		if (_Utils_cmp(
			...
			var hour = A2($elm$time$Time$toHour, timeModel.zone, timePosix);
			return (hour > 12) ? ($author$project$Todo2$timeLeadingZero(hour - 12) + (':' + $author$project$Todo2$timeLeadingZero(minute))) : ((!hour) ? ('12:' + $author$project$Todo2$timeLeadingZero(minute)) : ($author$project$Todo2$timeLeadingZero(hour) + (':' + $author$project$Todo2$timeLeadingZero(minute))));
		}
	});
var $author$project$Todo2$timeSelectionGenerator = function (model) {
	var selectableTimes = $author$project$Todo2$getSelectableTimes(model);
	var selectableTimesHtml = A2(
		$elm$core$List$map,
		function (newStartTime) {
			return A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$Events$onMouseOver(
						$author$project$Todo2$NewTodoStartTime(newStartTime))
					]),
				_List_fromArray(
					[
						A2(
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$checked(
								_Utils_eq(model.newTodo.startTime, newStartTime)),
								$elm$html$Html$Attributes$name('timeSelector'),
								$elm$html$Html$Attributes$type_('radio')
							]),
						_List_Nil),
						$elm$html$Html$text(
						A2(
							$author$project$Todo2$timeStringGenerator,
							model,
							$elm$time$Time$millisToPosix(newStartTime)))
					]));
		},
		selectableTimes);
	return A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$Attributes$id('timeSelection'),
				A2($elm$html$Html$Attributes$style, 'display', 'inline-block'),
				A2($elm$html$Html$Attributes$style, 'width', '100%'),
				A2($elm$html$Html$Attributes$style, 'height', '1.3em'),
				A2($elm$html$Html$Attributes$style, 'white-space', 'nowrap'),
...
var $author$project$Todo2$overdueGenerator = function (todo) {
	return A2(
		$elm$html$Html$li,
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('todo'),
				$elm$html$Html$Attributes$class('overdue')
			]),
		_List_fromArray(
			[
				A2(
							...
							[
								...
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												$elm$html$Html$text(todo.content)
											]))
									])),
								A2(
								$elm$html$Html$td,
								...
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												$elm$html$Html$Events$onClick(
												$author$project$Todo2$RemoveTodo(todo))
											]),
										_List_fromArray(
											[
...
var $author$project$Todo2$todoEditor = F3(
	function (todo, model, newContent) {
		var reducedTodoList = A2(
			$elm$core$List$filter,
			function (oldTodo) {
				return !_Utils_eq(oldTodo, todo);
			},
			model.todoList);
		var editedTodoList = A2(
			$elm$core$List$cons,
			_Utils_update(
				todo,
				{content: newContent}),
			reducedTodoList);
		return $author$project$Todo2$UpdateTodoList(editedTodoList);
	});
var $author$project$Todo2$todoGenerator = function (_v0) {
	var todo = _v0.a;
	var model = _v0.b;
	var todoEndTime = todo.startTime + todo.length;
	var time = model.time;
	var millisRemaining = todoEndTime - $elm$time$Time$posixToMillis(time.now);
	var countdownHtml = (_Utils_cmp(millisRemaining, todo.length) < 0) ? A2(
		$elm$html$Html$div,
		_List_Nil,
		_List_fromArray(
			[
				$elm$html$Html$text(
				$author$project$Todo2$countdownStringGenerator(millisRemaining))
			])) : A2(
		$elm$html$Html$div,
		_List_Nil,
		_List_fromArray(
			[
				...
				'Do by: ' + A2(
					$author$project$Todo2$timeStringGenerator,
					model,
					$elm$time$Time$millisToPosix(todoEndTime)))
			]));
	return A2(
		$elm$html$Html$li,
		_List_fromArray(
								...
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												$elm$html$Html$Events$onInput(
												A2($author$project$Todo2$todoEditor, todo, model))
											]),
										_List_fromArray(
											[
												$elm$html$Html$text(todo.content)
										...
										_List_fromArray(
											[
												...
												'Start by: ' + A2(
													$author$project$Todo2$timeStringGenerator,
													model,
													$elm$time$Time$millisToPosix(todo.startTime)))
											]))
									])),
								A2(
								$elm$html$Html$td,
...
var $author$project$Todo2$todoListGenerator = function (model) {
	if (!$elm$core$List$length(model.todoList)) {
		return A2(
			$elm$html$Html$div,
			_List_Nil,
			_List_fromArray(
		...
		var overDueList = A2(
			...
			function (overdueTodo) {
				return _Utils_cmp(
					overdueTodo.startTime + overdueTodo.length,
					$elm$time$Time$posixToMillis(model.time.now)) < 0;
			},
			model.todoList);
		var futureList = A2(
			$elm$core$List$filter,
			function (overdueTodo) {
				return !A2($elm$core$List$member, overdueTodo, overDueList);
			},
			model.todoList);
		return A2(
			$elm$html$Html$div,
			...
			_List_fromArray(
				[
					...
					$elm$html$Html$ol,
					_List_Nil,
					A2($elm$core$List$map, $author$project$Todo2$overdueGenerator, overDueList)),
					A2(
					$elm$html$Html$ol,
					_List_Nil,
					A2(
						$elm$core$List$map,
						$author$project$Todo2$todoGenerator,
						A2(
							$elm$core$List$map,
							function (todo) {
								return _Utils_Tuple2(todo, model);
							},
							futureList)))
				]));
	}
...
var $author$project$Todo2$view = function (model) {
	var whiteboard = A2(
		$elm$html$Html$span,
		_List_fromArray(
			[
		...
		_List_Nil);
	var newTodo = A2($author$project$Todo2$cleanNewTodo, model.newTodo, model);
	return {
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$class('todoList'),
						...
						_List_fromArray(
							[
										...
										_List_fromArray(
											[
												...
												'Todo : ' + A2(
													$author$project$Todo2$timeStringGenerator,
													model,
													$elm$time$Time$millisToPosix(newTodo.startTime)))
											]))
									])),
								A2(
								$elm$html$Html$main_,
									...
									[
										...
										_List_fromArray(
											[
												...
												_List_fromArray(
													[
														$elm$html$Html$Attributes$value(model.newTodo.content),
														$elm$html$Html$Events$onInput($author$project$Todo2$NewTodoContent)
													]),
												_List_Nil),
												A2(
												$elm$html$Html$span,
												...
												_List_fromArray(
													[
														...
														_List_fromArray(
															[
																$author$project$Todo2$lengthSelectionGenerator(model)
															]))
													])),
												A2(
												$elm$html$Html$span,
												...
												_List_fromArray(
													[
														...
														_List_fromArray(
															[
																$author$project$Todo2$timeSelectionGenerator(model)
															]))
													])),
												A2(
												$elm$html$Html$button,
												_List_fromArray(
													[
														$elm$html$Html$Events$onClick(
														$author$project$Todo2$SubmitNewTodo(newTodo))
													]),
												_List_fromArray(
													[
														$elm$html$Html$text('+')
								...
								_List_fromArray(
									[
										$author$project$Todo2$todoListGenerator(model)
									]))
							])),
...
var $author$project$Todo2$main = $elm$browser$Browser$document(
	{init: $author$project$Todo2$init, subscriptions: $author$project$Todo2$subscriptions, update: $author$project$Todo2$update, view: $author$project$Todo2$view});
_Platform_export({'Todo2':{'init':$author$project$Todo2$main(
